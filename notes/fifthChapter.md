...
# 如何求解金矿问题
本小节篇幅过长，不宜整理。  
总的来说这道题就是动态规划，跟背包问题类似。  
小灰大黄哟哟切克闹

# 寻找缺失的整数
本小节是最后一个小结，所以小灰越战越勇，不断解决问题，面试官就不断提新问题  

### 题目1
在一个无序数组里有99个不重复的正整数，范围是1～100，唯独缺少1～100中的整数。如何找出这个缺失的整数？

### 小灰解决1
创建哈希表，以1到100这100个整数为Key，然后遍历整个数组，每读到一个整数，就定位到哈希表中对应的Key，然后删除这个Key，最终剩下的Key
就是缺失的那个整数。  
分析：假设数组长度是n，那么该解法的时间复杂度是***O(1)***，空间复杂度是***O(n)***。  

### 小灰解法2
先把数组元素从小到大排序，然后遍历，如果发现某两个相邻的元素不连续，说明缺少的就是这两个元素之间的整数。  
分析：假设数组长度是n，如果用时间复杂度为O(nlogn)的排序算法进行排序，那么该解法的时间复杂度是***O(nlogn)***，空间复杂度是***O
(1)***。

### 小灰解法3
先算出1+2+3+...+100的和，然后依次减去数组里的元素，最后得到的差值就是缺失的整数。  
分析：假设数组长度是n，那么该解法的时间复杂度是***O(n)***，空间复杂度是***O(1)***。

### 题目2(扩展)
一个无序数组里有若干个正整数，范围是1～100，其中99个整数都出现了偶数次，只有1个整数出现了奇数次，如何找到这个出现奇数次的整数？

提示：异或运算，进行位运算时，相同位得0，不同位得1。

### 小灰解法
遍历整个数组，依次做异或运算。由于异或运算在进行位运算时，相同为0，不同为1，因此所有出现偶数次的整数都会相互抵消变成0
，只有唯一出现奇数次的整数都会被留下。  
分析：假设数组长度是n，那么该解法的时间复杂度是***O(n)***，空间复杂度是***O(1)***。

### 题目3(扩展)
假设一个无序数组里有若干个正整数，范围是1～100，其中98个整数出现了偶数次，只有2个整数出现了奇数次，如何找到这2个出现奇数次的整数？

提示：用上一题的解法俨然不够，需要分治法。

### 小灰解法
把2个出现了奇数次的整数命名为A和B。遍历整个数组，然后依次做异或运算，进行异或运算的最终结果，等同于A和B
进行异或运算的结果。在这个结果中，至少会有一个二进制位是1(如果都是0，说明A和B相等，和题目不符)。  
分析：假设数组长度是n，那么该解法的时间复杂度是***O(n)***。把数组分成两部分，并不需要借助额外的存储空间，完全可以在按二进制位分组的同时来做异或运算，所以空间复杂度仍然是***O(1)***。
